...
class QJsonIO
{
    static QJsonValue SetValueAtPath(const QJsonValue &node, const QJsonValue &value, const QJsonIOPath &path, qsizetype offset)
    {
        if (offset == path.size())
            return value;

        const auto &[key, type] = path.at(offset);
        if (type == QJsonIOPathType::JSONIO_MODE_ARRAY)
        {
            auto array = node.isArray() ? node.toArray() : QJsonArray{};
            const auto index = key.toInt();
            while (array.size() <= index)
                array.append(QJsonValue{});
            array[index] = SetValueAtPath(array.at(index), value, path, offset + 1);
            return array;
        }

        auto object = node.isObject() ? node.toObject() : QJsonObject{};
        object[key] = SetValueAtPath(object.value(key), value, path, offset + 1);
        return object;
    }
  ...
  public:
    ...
    static void SetValue(parent_type &parent, const value_type &t, const QJsonIOPath &path)
    {
        ...
        const auto lastNode = SetValueAtPath(QJsonValue(parent), QJsonValue::fromVariant(QVariant::fromValue(t)), path, 0);
